...
PHASE = "training"
OUTPUT_FOLDER = os.path.join("/home/alex/thesis_ssd/CARLA_nightly/test_test/_out2", PHASE)
...
def save_training_files(player, image, datapoints, point_cloud,intrinsic,extrinsic,captured_frame_no):
    logging.warning("Test2 Attempting to save frame no: {}".format(
        captured_frame_no))
...
def saving_time(prev_time):
    ...
    if now % SAVE_GAP == 0 and now != prev_time:
        print(now)
        print(prev_time)
        print("Test2 Saving data at " + (time.asctime( time.localtime(now ))))
        return now
    else:
        return None

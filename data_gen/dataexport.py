...
def save_locational(filename,player):
    with open(filename, 'w') as f:
        loc=player.get_transform().location
        rot=player.get_transform().rotation
        #x y z pitch yaw roll
        out_str = "%.6f %.6f %.6f %.6f %.6f %.6f" %(loc.x, loc.y, loc.z, rot.pitch, rot.yaw, rot.roll)
        f.write(out_str)
    logging.info("Wrote locational data to %s", filename)

...
PHASE = "training"
OUTPUT_FOLDER = os.path.join("_out32111", PHASE)
...
def sensor_setting(world):
    ...
    lidar_bp.set_attribute('range', str(MAX_RENDER_DEPTH_IN_METERS))	#cm to m
...
def processing(world,image_rgb, image_depth, image_lidar, intrinsic, player,agents,camera_to_car_transform,lidar_to_car_transform,gen_time):
     if image_rgb is not None and image_depth is not None:
        ...
        print(depth_to_array(image_depth))
...
def save_training_files(player, image, datapoints, point_cloud,intrinsic,extrinsic,captured_frame_no):
    logging.warning("Attempting to save frame no: {}".format(
        captured_frame_no))
...
def saving_time(prev_time):
    ...
    if now % SAVE_GAP == 0 and now != prev_time:
        print(now)
        print(prev_time)
        print("Saving data at " + (time.asctime( time.localtime(now ))))
        return now
    else:
        return None
...
def main():
    ...
    try:
        ...
        #start_pose = carla.Transform(carla.Location(x=9, y=-120))
        ...
        #with CarlaSyncMode(world, camera_rgb, fps=30) as sync_mode:
            ...
            while True:
                ...
                snapshot, image_lidar, image_rgb, image_depth = sync_mode.tick(timeout=2.0)
                ...
                if now is not None and point_cloud.shape[0]>0 and datapoints is not None:
                    np.save("/home/alex/Desktop/depthcam",np.array(depth_to_array(image_depth)))
